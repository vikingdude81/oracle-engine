"""
Trajectory Analysis Wrapper for Consciousness Circuit
=====================================================

Combines consciousness measurement with trajectory dynamics from:
- helios_metrics: HeliosAnomalyScope, Lyapunov, Hurst, MSD
- tame_metrics: Goal-directedness, agency, attractor dynamics
...
from .universal import UniversalCircuit, UniversalResult
...
from .plugins import TrajectoryPlugin, ChaosPlugin, AgencyPlugin
...
class TrajectoryAnalysisResult:
    ...
    def interpretation(self) -> str:
        ...
        # Trajectory dynamics
        if self.trajectory_class == SignalClass.ATTRACTOR.value:
            lines.append("🎯 ATTRACTOR-LOCKED - converging to coherent reasoning pattern")
        elif self.trajectory_class == SignalClass.BALLISTIC.value:
            lines.append("🚀 BALLISTIC motion - directed, purposeful thought")
        elif self.trajectory_class == SignalClass.CHAOTIC.value:
            lines.append("🌀 CHAOTIC dynamics - exploring solution space")
        elif self.trajectory_class == SignalClass.DIFFUSIVE.value:
            lines.append("🔄 DIFFUSIVE motion - random walk, uncertain")
        elif self.trajectory_class == SignalClass.DRIFT.value:
...
class ConsciousnessTrajectoryAnalyzer:
    ...
    def _analyze_trajectory(self, hidden_states: np.ndarray) -> Dict[str, Any]:
        ...
        trajectory_class = classify_trajectory(msd)
